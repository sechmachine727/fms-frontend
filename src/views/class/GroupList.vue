...
const querySearchFromUrl = () => {
  ...
  if (
    ...
    getQueryStatusFromUrl.value !== '' ||
    getQueryTechnicalGroupFromUrl.value !== '' ||
    getQuerySiteFromUrl.value !== ''
  ) {
    ...
    if (getQueryTechnicalGroupFromUrl.value !== '') {
      ...
      technicalGroupOptionsSearch.value = technicalGroups.value.filter((item1) =>
        values.some((item2) => item1.technicalGroupCode === item2.technicalGroupCode)
      )
    }

    if (getQuerySiteFromUrl.value !== '') {
      const values = getQuerySiteFromUrl.value.split(',').map((site) => ({ code: site }))
      siteOptionsSearch.value = sites.value.filter((item1) =>
        values.some((item2) => item1.site === item2.site)
      )
    }

    if (getQueryStatusFromUrl.value !== '') {
      const values = getQueryStatusFromUrl.value.split(',').map((status) => ({ id: status }))
      statusOptions.value = getStatusArray().filter((item1) =>
        values.some((item2) => item1.id === item2.id)
      )
    }
    applyFilters()
  }
...
const buildQueryObject = () => {
  ...
  if (technicalGroupOptionsSearch.value.length > 0) {
    const technicalGroupCode = technicalGroupOptionsSearch.value.map(
