...
const querySearchFromUrl = () => {
  ...
  if (
    ...
    getQueryTechnicalGroupFromUrl.value !== ''
  ) {
    ...
    if (getQueryTechnicalGroupFromUrl.value !== '') {
      const values = getQueryTechnicalGroupFromUrl.value
        .split(',')
        .map((technical) => ({ code: technical }))
      technicalGroupOptionsSearch.value = technicalGroups.value.filter((item1) =>
        values.some((item2) => item1.code === item2.code)
      )
    }
    applyFilters()
  }
  ...
  confirm.require({
    ...
    accept: () => {
      trainingProgramStore
        .fetchToggleToActive(selectedItem.value.id)
        .then(() => {
          toast.add({
            severity: 'success',
            summary: 'Training Program successfully activated',
            life: 3000
          })
...
const confirmDeactive = (value) => {
  confirm.require({
    ...
    accept: () => {
      trainingProgramStore
        .fetchToggleActivate(selectedItem.value.id)
        .then(() => {
          trainingProgramStore.fetchTrainingPrograms().then(() => {
            trainingPrograms.value = trainingProgramStore.trainingPrograms
          })
          toast.add({
            severity: 'success',
            summary: 'Training Program successfully deactivated',
            life: 3000
          })
        })
        .catch((errors) => {
...
const confirmReject = (value) => {
  confirm.require({
    ...
    accept: () => {
      trainingProgramStore
        .fetchToggleReviewingToDeclined(selectedItem.value.id, declineReason.value)
        .then(() => {
          trainingProgramStore.fetchTrainingPrograms().then(() => {
            trainingPrograms.value = trainingProgramStore.trainingPrograms
          })
          toast.add({
            severity: 'success',
            summary: 'Training Program declined successfully',
            life: 3000
          })
        })
        .catch((errors) => {
...
onMounted(() => {
  trainingProgramStore.fetchTrainingPrograms().then(() => {
    trainingPrograms.value = trainingProgramStore.trainingPrograms
    topicInfoList.value = trainingProgramStore.trainingPrograms
      .map((program) => {
