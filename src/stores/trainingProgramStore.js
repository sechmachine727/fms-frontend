...
export const useTrainingProgramStore = defineStore('trainingProgramStore', {
  ...
  actions: {
    ...
    async fetchTrainingProgramsByTechnicalGroup(id) {
      try {
        this.trainingProgramsByTecnicalGroups =
          await trainingProgramApi.getAllByTechnicalGroupId(id)
      } catch (error) {
        console.error('Failed to fetch trainees', error)
      }
    },
    ...
    fetchFilterTrainingPrograms(criteria) {
      const departmentNames = criteria.departmentOptionsSearch.map((department) =>
        department.departmentName.toLowerCase()
      )
      const technicalGroup = criteria.technicalGroupOptionsSearch.map((technical) =>
        technical.code.toLowerCase()
      )
      const searchQuery =
